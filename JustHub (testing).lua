...
function SectionMethods:addScriptBox(o)
	...
	if o.Role and not JustHub:CheckRole(o.Role) then
	...
	addBorder(f, theme["Color Stroke"], 1)
...
function JustHub:CreateWindow(o)local o=o or{} local w=o.Name or"JustHub Window" local t=getCurrentTheme(o.Theme) local g=HttpService:GenerateGUID(false) local f=w.." ["..g.."]" local p=Players.LocalPlayer local pg=p:WaitForChild("PlayerGui") local sg=createInstance("ScreenGui",{Name="JustHub",ResetOnSpawn=false},pg) self.ScreenGui=sg local uw=JustHub.Save.UISize[1] local uh=JustHub.Save.UISize[2] local mF=createInstance("Frame",{Name="MainFrame",AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,-0.5,0),Size=UDim2.new(0,uw,0,uh),BackgroundColor3=t["Color Hub 2"]},sg) createInstance("UICorner",{CornerRadius=UDim.new(0,12)},mF) addBorder(mF,t["Color Stroke"],2) local tB=createInstance("Frame",{Name="TopBar",Size=UDim2.new(1,0,0,60),BackgroundColor3=t["Color Hub 2"]},mF) createInstance("UICorner",{CornerRadius=UDim.new(0,12)},tB) local tL=createInstance("TextLabel",{Name="TitleLabel",Size=UDim2.new(1,-180,1,0),Position=UDim2.new(0,10,0,0),BackgroundTransparency=1,Text=f,TextColor3=t["Color Text"],Font=Enum.Font.GothamBold,TextSize=18,TextXAlignment=Enum.TextXAlignment.Left},tB) local wL=createInstance("TextLabel",{Name="WelcomeLabel",Size=UDim2.new(0,150,0,20),AnchorPoint=Vector2.new(0,1),Position=UDim2.new(0,10,1,-30),BackgroundTransparency=1,Text="Welcome, "..p.Name,TextColor3=t["Color Text"],Font=Enum.Font.GothamBold,TextSize=16,TextXAlignment=Enum.TextXAlignment.Left},mF) local function ccb(n,s,p)return createInstance("TextButton",{Name=n,Text=s,Size=UDim2.new(0,40,0,40),Position=p,BackgroundTransparency=1,TextColor3=t["Color Text"],Font=Enum.Font.GothamBold,TextSize=24},tB)end local hB=ccb("HideButton","–",UDim2.new(1,-110,0,10)) local xB=ccb("MaxButton","□",UDim2.new(1,-70,0,10)) local cB=ccb("CloseButton","X",UDim2.new(1,-30,0,10)) local sw=JustHub.Save.TabSize local sB=createInstance("Frame",{Name="Sidebar",Size=UDim2.new(0,sw,1,-60),Position=UDim2.new(0,0,0,60),BackgroundColor3=t["Color Hub 2"]},mF) createInstance("UICorner",{CornerRadius=UDim.new(0,10)},sB) createInstance("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,10),HorizontalAlignment=Enum.HorizontalAlignment.Center,VerticalAlignment=Enum.VerticalAlignment.Top},sB) local cC=createInstance("Frame",{Name="ContentContainer",Size=UDim2.new(1,-sw,1,-60),Position=UDim2.new(0,sw,0,60),BackgroundColor3=t["Color Hub 2"]},mF) createInstance("UICorner",{CornerRadius=UDim.new(0,10)},cC) local sF=createInstance("ScrollingFrame",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ScrollBarThickness=8,BorderSizePixel=0},cC) createInstance("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,10),HorizontalAlignment=Enum.HorizontalAlignment.Left},sF) local fpsL=createInstance("TextLabel",{Name="FPSLabel",Size=UDim2.new(0,100,0,20),AnchorPoint=Vector2.new(1,1),Position=UDim2.new(1,-10,1,-10),BackgroundTransparency=1,TextColor3=t["Color Text"],Font=Enum.Font.Gotham,TextSize=14,Text="FPS: Calculating..."},mF) RunService.Heartbeat:Connect(function(d)local fPS=math.floor(1/d) fpsL.Text="FPS: "..fPS end) local iin=TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.Out) local tin=TweenService:Create(mF,iin,{Position=UDim2.new(0.5,0,0.5,0)}) tin:Play() local min=false local max=false local osz=mF.Size local op=mF.Position hB.MouseButton1Click:Connect(function()if not min then tweenProperty(mF,{Size=UDim2.new(osz.X.Scale,osz.X.Offset,0,60)},0.3) wL.Visible=false sB.Visible=false cC.Visible=false fpsL.Visible=false min=true else tweenProperty(mF,{Size=osz},0.3) wait(0.3) wL.Visible=true sB.Visible=true cC.Visible=true fpsL.Visible=true min=false end end) xB.MouseButton1Click:Connect(function()if not max then tweenProperty(mF,{Size=UDim2.new(1,0,1,0),Position=UDim2.new(0.5,0,0.5,0)},0.3) max=true else tweenProperty(mF,{Size=osz,Position=op},0.3) max=false end end) cB.MouseButton1Click:Connect(function() local tout=TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.In) local ctw=TweenService:Create(mF,tout,{Position=UDim2.new(0.5,0,-0.5,0)}) ctw:Play() ctw.Completed:Connect(function() mF.Visible=false local pg2=Players.LocalPlayer:WaitForChild("PlayerGui") local sg2=createInstance("ScreenGui",{Name="ShowUI",ResetOnSpawn=false},pg2) local sbt=createInstance("TextButton",{Name="ShowUIButton",Size=UDim2.new(0,150,0,50),Position=UDim2.new(0.5,-75,0,0),BackgroundColor3=t["Color Hub 2"],Text="Show UI",TextColor3=Color3.fromRGB(128,0,128),Font=Enum.Font.GothamBold,TextSize=20},sg2) createInstance("UICorner",{CornerRadius=UDim.new(0,25)},sbt) createInstance("UIStroke",{Color=t["Color Theme"],Thickness=1},sbt) sbt.MouseButton1Click:Connect(function() mF.Visible=true tweenProperty(mF,{Position=UDim2.new(0.5,0,0.5,0)},0.5) sg2:Destroy() end) end) end) mF.Active=true local function dR(f)local d=false local ds,sp f.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then d=true ds=i.Position sp=f.Position end end) f.InputChanged:Connect(function(i) if d and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then local delta=i.Position-ds local np=UDim2.new(sp.X.Scale,sp.X.Offset+delta.X,sp.Y.Scale,sp.Y.Offset+delta.Y) np=clampPosition(np,Vector2.new(sg.AbsoluteSize.X,sg.AbsoluteSize.Y)) f.Position=np end end) UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then d=false end end) end dR(mF) local function rS(f,g)local r=false local ds,ss g.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then r=true ds=i.Position ss=f.Size end end) g.InputChanged:Connect(function(i) if r and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then local delta=i.Position-ds local nw=math.max(300,ss.X.Offset+delta.X) local nh=math.max(200,ss.Y.Offset+delta.Y) f.Size=UDim2.new(0,nw,0,nh) end end) UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then r=false end end) end local rg=createInstance("Frame",{Name="ResizeGrip",Size=UDim2.new(0,20,0,20),Position=UDim2.new(1,-20,1,-20),BackgroundColor3=Color3.fromRGB(50,50,50),BackgroundTransparency=0.3,BorderSizePixel=0},mF) createInstance("UICorner",{CornerRadius=UDim.new(0,5)},rg) rS(mF,rg) local nc=createInstance("Frame",{Name="NotificationContainer",AnchorPoint=Vector2.new(1,1),Position=UDim2.new(1,-10,1,-10),Size=UDim2.new(0,300,1,-20),BackgroundTransparency=1},sg) createInstance("UIListLayout",{Padding=UDim.new(0,8),HorizontalAlignment=Enum.HorizontalAlignment.Right,VerticalAlignment=Enum.VerticalAlignment.Bottom,SortOrder=Enum.SortOrder.LayoutOrder},nc) self.NotificationContainer=nc local wObj={ScreenGui=sg,MainFrame=mF,TopBar=tB,Sidebar=sB,ContentContainer=cC,Tabs={}} function wObj:addTab(nm) nm=nm or"Tab" local b=createInstance("TextButton",{Name=nm.."Button",Text=nm,Size=UDim2.new(1,0,0,20),BackgroundColor3=t["Color Stroke"],TextColor3=Color3.fromRGB(128,0,128),Font=Enum.Font.GothamBold,TextSize=12},sB) createInstance("UICorner",{CornerRadius=UDim.new(0,10)},b) createInstance("UIStroke",{Color=t["Color Theme"],Thickness=1},b) local tc=createInstance("Frame",{Name=nm.."Content",Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Visible=false},sF) local tobj={Name=nm,Button=b,Content=tc,Sections={}} table.insert(wObj.Tabs,tobj) b.MouseButton1Click:Connect(function() for _,v in ipairs(wObj.Tabs)do v.Content.Visible=false end tobj.Content.Visible=true end) if #wObj.Tabs==1 then tobj.Content.Visible=true end function tobj:addSection(sn,sh) sn=sn or"Section" sh=sh or80 local sframe=createInstance("Frame",{Name=sn,Size=UDim2.new(1,0,0,sh),BackgroundColor3=t["Color Hub 2"],BackgroundTransparency=0.0},tc) createInstance("UICorner",{CornerRadius=UDim.new(0,8)},sframe) local st=createInstance("TextLabel",{Name="SectionTitle",Text=sn,Size=UDim2.new(1,0,0,30),BackgroundTransparency=1,TextColor3=t["Color Text"],Font=Enum.Font.GothamBold,TextSize=14,TextXAlignment=Enum.TextXAlignment.Left},sframe) local sc=createInstance("Frame",{Name="SectionContent",Size=UDim2.new(1,0,1,-30),Position=UDim2.new(0,0,0,30),BackgroundTransparency=1},sframe) createInstance("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,Padding=UDim.new(0,5),SortOrder=Enum.SortOrder.LayoutOrder,HorizontalAlignment=Enum.HorizontalAlignment.Left},sc) local sObj={Frame=sframe,Title=st,Content=sc} table.insert(tobj.Sections,sObj) setmetatable(sObj,{__index=SectionMethods}) return sObj end return tobj end return wObj end
